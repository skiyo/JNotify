...
public class JNotify_linux {
	...
	static {
		...
		if (System.getProperty("os.arch").equals("amd64")) {
			Util.loadLibrary("libjnotify64.so");
		} else {
			Util.loadLibrary("libjnotify.so");
		}
		int res = nativeInit();
		if (res != 0) {
			throw new RuntimeException(
					"Error initializing fshook_inotify library. linux error code #"
							+ res + ", man errno for more info");
		}
		init();
	}
	 ...
	 */
	public final static int IN_ALL_EVENT = (IN_ACCESS | IN_MODIFY | IN_ATTRIB
	...
	public static int addWatch(String path, int mask) throws JNotifyException {
		...
		if (wd < 0) {
			throw new JNotifyException_linux("Error watching " + path + " : "
					+ getErrorDesc(-wd), -wd);
		}

		debug(wd + " = JNotify_linux.addWatch(" + path + ","
				+ getMaskDesc(mask) + ")");
	...
	static void callbackProcessEvent(String name, int wd, int mask, int cookie) {
		debug("JNotify.event(name=" + name + ", wd=" + wd + ", "
				+ getMaskDesc(mask) + (cookie != 0 ? ", cookie=" + cookie : "")
				+ ")");
