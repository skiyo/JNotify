...
public class JNotify_win32 {
	static {
		try {
			...
			*/
			...
			if (System.getProperty("os.arch").equals("amd64")) {
				Util.loadLibrary("jnotify_64bit.dll");
			} else {
				Util.loadLibrary("jnotify.dll");
			}
			
		} catch (UnsatisfiedLinkError e) {
			System.err.println("Error loading library, java.library.path="
					+ System.getProperty("java.library.path"));
			throw e;
		}
		int res = nativeInit();
		if (res != 0) {
			throw new RuntimeException("Error initialiing native library. (#"
					+ res + ")");
		}
	}
	...
	private static native int nativeAddWatch(String path, long mask,
	...
	public static int addWatch(String path, long mask, boolean watchSubtree)
			throws JNotifyException {
		int wd = nativeAddWatch(path, mask, watchSubtree);
		if (wd < 0) {
			throw new JNotifyException_win32(getErrorDesc(-wd) + " : " + path,
					-wd);
		}
		return wd;
	}
	...
	public static void callbackProcessEvent(int wd, int action,
			String rootPath, String filePath) {
